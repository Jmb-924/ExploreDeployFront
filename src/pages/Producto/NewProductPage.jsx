...
import { faPlus, faRecycle, faSquareMinus, faTrashCanArrowUp } from '@fortawesome/free-solid-svg-icons'
import { formatNumPrecio } from '../../utils/helpers/NumFormat'
import { postSecuredNotJson } from '../../services/api/MongoDb/PostMethod'
import { ReplaceSpaceTo } from '../../utils/helpers/ReplaceBlankSpace'

const NewProductPage = () => {
   const { userAuth } = useAuthContext()
   // const { setShowAlert, setMsgAlert } = useAlertContext()
   ...
   const openFiles = () => {
      ...
      document.getElementById('fileInput').addEventListener('change', function (event) {
         ...
         if (images.length >= 1) {
            ...
            setPreviewUrls([...previewUrls, ...previewArray])
         } else {
   ...
   const removeImage = (id) => {
      const img = images.filter((_, index) => index !== id)
      const preImg = previewUrls.filter((_, index) => index !== id)
      setImages(img)
      setPreviewUrls(preImg)
   }
   ...
   const sendForm = async (e) => {
      ...
      const a = await postSecuredNotJson('/productos/crear', userAuth?.userInfo, data)
      if (a.type === "Success:") {
         //    // setMsgAlert({
         //    //    mensaje: a?.mensaje,
         //    //    status: a?.status,
